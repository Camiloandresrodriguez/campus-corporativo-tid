...
export default function SeccionMensajesDirectos() {
  ...
  console.log("BASE:", baseContactos);
  console.log("FILTRADOS:", contactosFiltrados);

  const mensajesDelChatActivo = chatActivo ? (historialChats[chatActivo.id] || []) : [];

  return (          
    <div style={{ display: 'flex', gap: 16, height: '420px', backgroundColor: '#FFFFFF', borderRadius: '12px', padding: 12,boxShadow: '0 8px 24px rgba(0,0,0,0.08)' }}>
      {/* Columna Izquierda: Buscador + Contactos */}
      <div style={{ width: '38%', borderRight:'1px solid #D9E2EC' , paddingRight: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
        
        {/* Input Buscador */}
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
          <Search size={14} color="#2563EB" style={{ position: 'absolute', left: 10 }} />
          <input 
            type="text" 
            ...
            style={{ 
              width: '100%',backgroundColor: '#FFFFFF', color: '#f1f5f9',  
              fontSize: '12px',border: '1px solid #D9E2EC', 
        ...
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>       
          {contactosFiltrados.length === 0 ? (
            <small style={{ color: '#000000', padding: 8, textAlign: 'center' }}>
            ...
            contactosFiltrados.map(u => (
              <div 
                ...
                style={{ 
                  ...
                  cursor: 'pointer', 
                  backgroundColor: chatActivo?.id === u.id ?'#E8F1FF'  : 'transparent', 
                  border: chatActivo?.id === u.id ? '1px solid #2563EB' : '1px solid transparent',
                  display: 'flex',
                  alignItems: 'center',
                  gap: 8,
                  overflow: 'hidden',
          ...
          <>
                ...
                mensajesDelChatActivo.map(m => (
                  <div key={m.id} style={{ display: 'flex', justifyContent: m.remitenteId === 'yo' ? 'flex-end' : 'flex-start' }}>
                    <div style={{ 
                      ...
                      backgroundColor: m.remitenteId === 'yo' ?  '#2563EB' :'#00AEC7' , color: '#ffffff' 
                    }}>
                      {m.texto}
                    </div>
                  </div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <input 
                ...
                style={{ flex: 1, backgroundColor:  '#FFFFFF', color: '#000000' , border:'1px solid #D9E2EC' , borderRadius: '12px', padding: '6px 12px' }} 
              />
