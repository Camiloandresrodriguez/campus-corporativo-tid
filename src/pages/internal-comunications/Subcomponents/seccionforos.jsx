...
>
        ...
        <div className="card" style={{ padding: 16, marginBottom: 20 }}>
          ...
          <h3 style={{ margin: '8px 0', fontSize: 18, color: '#0f172a' }}>{foroSeleccionado.titulo}</h3>
          <small style={{ color: '#64748b' }}>Iniciado por {foroSeleccionado.autor}</small>
        </div>

        {/* Sección de aportes / comentarios */}
        <h4 style={{ fontSize: 14, color: '#0f172a', marginBottom: 12 }}>Respuestas de la Comunidad ({foroSeleccionado.comentarios.length})</h4>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
          {foroSeleccionado.comentarios.length === 0 ? (
            ...
            foroSeleccionado.comentarios.map(c => (
              <div key={c.id} style={{ backgroundColor:  '#dbeafe', padding: 12, borderRadius: 8, borderLeft: '3px solid #475569' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <strong style={{ fontSize: 12, color: '#10b981' }}>{c.usuario}</strong>
                  <small style={{ fontSize: 11, color: '#6b7280' }}>{c.fecha}</small>
                </div>
                <p style={{ margin: 0, fontSize: 13, color: '#64748b' }}>{c.texto}</p>
              </div>
            ))
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input 
            ...
            style={{ flex: 1, backgroundColor: '#f1f5f9', color:  '#060b12'}}
          />
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {hilos.map((hilo) => (
          ...
          >
            {/* Sistema de Votos */}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor:  '#dbeafe', padding: '6px 10px', borderRadius: 8 }}>
              <ArrowUp 
                size={16} 
                color="#10b981" 
              ...
              />
              <span style={{ fontSize: 13, fontWeight: 'bold', color: '#140707' }}>{hilo.votos}</span>
            </div>

            {/* Datos del Hilo */}
            <div style={{ flex: 1 }}>
              <span className="badge badge-gray" style={{ fontSize: 10 }}>{hilo.categoria}</span>
              <h4 style={{ margin: '4px 0', fontSize: 15, color: '#0f172a', fontWeight: 700 }}>{hilo.titulo}</h4>
              <small style={{ color: '#060b12' }}>Iniciado por {hilo.autor}</small>
            </div>

            {/* Contador de respuestas */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#2d5081', fontSize: 13 }}>
